...
Desde que la Acción A tiene un segundo escalón (`models.HARINA_ALIMENTAR`: 30% de harina por +2, ACTIONS_REGISTRY.md §3), la carta además se puede **contrarrestar del todo** pagando tres tokens en vez de uno: alimentar +2 → 4, Aletargamiento `-2` → 2. La Vitalidad inicial no cuenta con ello — quien no pueda pagar el escalón caro sigue sobreviviendo desde 2 con el barato — pero convierte el Aletargamiento en una decisión de harina y no solo en un golpe que encajar.