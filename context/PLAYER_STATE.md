...
* `puntos_accion` (Integer): PA disponibles en el turno actual. (Máximo 2, se reinician en cada Fase II. Puede subir a 3 temporalmente usando "Horas Extras", que además entrega el marcador neutral — ver `horas_extras_usadas`).
...
* `horas_extras_usadas` (Boolean): Bandera que se reinicia a `False` al inicio de cada Fase II (junto con `puntos_accion`) para limitar la acción auxiliar "Horas Extras" a 1 vez por día. Marca dos cosas a la vez: el +1 PA ya cobrado y el **marcador neutral** que viene con él (ACTIONS_REGISTRY.md §3 «Horas Extras»).
* `espacio_repetido_hoy` (Optional[String], `@property`): Id del espacio en el que este jugador gastó hoy su marcador neutral, o `None`. Se deriva de la única entrada duplicada que puede tener `acciones_pa_usadas_hoy`. Al ser un `@property`, `dataclasses.asdict` no lo trae: `server/views.py` lo inyecta para que el tablero dibuje el peón gris.
* `marcador_neutral_disponible` (Boolean, `@property`): `horas_extras_usadas and espacio_repetido_hoy is None`. Es lo que consulta `ActionManager._require_espacio_disponible` para dejar pasar una segunda visita, y `disponibilidad.py` para mantener esa casilla encendida.
* `estasis_suspendida` (Boolean): Bandera que indica si el jugador ha suspendido su Estasis Biológica para la Fase III de HOY, es decir, si su cultivo base va a sufrir el desgaste metabólico normal esta noche pese a tener la Criopreservación instalada (ACTIONS_REGISTRY.md §3 «Estasis Biológica»). Solo tiene sentido con `criopreservacion == True`; en cualquier otro jugador es inerte. La limpia la **propia Fase III** tras aplicar el desgaste, no el inicio del día, de modo que la Estasis se reactiva sola y un ajuste olvidado no puede contaminar a nadie. **No es un marcador de "ya usada"**: la acción es un interruptor de dos sentidos y este campo no participa en `GameEngine._jugador_elegible`.
* `acciones_pa_usadas_hoy` (List[String]): Una entrada por **visita** a un espacio de acción con costo de PA (B, C, D, E, F, G, H, I, "simposio") en este Día de Laboratorio — cada espacio solo puede visitarse una vez por día (ACTIONS_REGISTRY.md §1). **Un id puede aparecer dos veces, y sólo uno:** es el marcador neutral de las Horas Extras, que permite repetir un espacio de `actions.ESPACIOS_CON_MARCADOR_NEUTRAL`. Guardarlo así, y no como campo propio, es lo que evita tocar `VERSION_FORMATO` — el mismo argumento que decidió los Pliegues y el Mostrador; ningún otro camino puede duplicar una entrada, porque `ocupar_espacio_accion` (Acción E, Descarte) conserva su precondición de "no estar ya en la lista" y esos dos espacios quedan fuera del conjunto. Todo el código lo consulta con `in`, así que el duplicado sólo lo ven las dos propiedades de arriba. Se llena al gastar el PA de la acción y se reinicia a `[]` al inicio de cada Fase II, junto con `accion_alimentar_usada`. Pedido de Urgencia (0 PA) y las acciones gratuitas (Alimentar, Horas Extras) no participan de esta lista — tienen sus propias banderas dedicadas. `"jefatura"` sí entra en la lista (gasta PA y ocupa el espacio del jugador), pero su límite real es **global** y vive en el motor (`GameEngine.jefatura_reclamada_por`): un solo jugador de la mesa puede reclamarla cada día. `"mostrador"` es el caso contrario y **nunca entra en la lista** pese a costar 1 PA: el Turno de Mostrador llama a `consumir_punto_accion(..., ocupa_espacio=False)` precisamente para poder repetirse el mismo día (ACTIONS_REGISTRY.md §2 «Mostrador»), ya que el turno hueco que viene a tapar puede darse una vez por cada PA.
