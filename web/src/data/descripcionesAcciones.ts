...
export const descripcionesAcciones: Record<IdAccion, string> = {
  ...
  horas_extras:
    'Gratis, en cualquier momento de tu turno, una vez por día. Costo: 1 Dato de Investigación. Otorga inmediatamente +1 Punto de Acción y un marcador neutral: una de tus acciones con costo de PA de hoy podrá caer en un espacio que ya visitaste (B, C, D, F, G, Simposio, H o I). El marcador solo se gasta al repetir, así que da igual cuándo pidas las Horas Extras: si el PA extra va a un espacio libre, pones tu color como siempre y te lo guardas para más tarde. No sirve para la Jefatura (se agota para toda la mesa, no solo para ti) ni para Pliegues o Descarte (cuestan 0 PA). El Mostrador ya se repite solo.',
  incubadora:
